type Cast256Block = [u32; 4];

fn bswap(n: u32) -> u32 {
    ((n.rotate_left(8) & 0x00ff00ff) | (n.rotate_right(8) & 0xff00ff00))
}
...
fn f1(k: &mut [u32; 8], y: usize, x: usize, kr: u32, km: u32) {
...
fn f2(k: &mut [u32; 8], y: usize, x: usize, kr: u32, km: u32) {
...
fn f3(k: &mut [u32; 8], y: usize, x: usize, kr: u32, km: u32) {
...
fn f_rnd(x: &mut [u32; 4], l_key: [u32; 96], n: u32) {
    let mut copy: [u32; 8] = [0; 8];
    copy[..4].copy_from_slice(&x[..]);
    f1(&mut copy, 2, 3, l_key[n as usize],       l_key[(n + 4) as usize]);
    f2(&mut copy, 1, 2, l_key[(n + 1) as usize], l_key[(n + 5) as usize]);
    f3(&mut copy, 0, 1, l_key[(n + 2) as usize], l_key[(n + 6) as usize]);
    f1(&mut copy, 3, 0, l_key[(n + 3) as usize], l_key[(n + 7) as usize]);
    x[..].copy_from_slice(&copy[..4]);
}

fn i_rnd(x: &mut [u32; 4], l_key: [u32; 96], n: u32) {
    let mut copy: [u32; 8] = [0; 8];
    copy[..4].copy_from_slice(&x[..]);
    f1(&mut copy, 3, 0, l_key[(n + 3) as usize], l_key[(n + 7) as usize]);
    f3(&mut copy, 0, 1, l_key[(n + 2) as usize], l_key[(n + 6) as usize]);
...
fn k_rnd(k: &mut [u32; 8], tr: [u32; 8], tm: [u32; 8]) {
    f1(k, 6, 7, tr[0], tm[0]);
    f2(k, 5, 6, tr[1], tm[1]);
    f3(k, 4, 5, tr[2], tm[2]);
    f1(k, 3, 4, tr[3], tm[3]);
...
pub fn get_key_schedule(key: [u32; 8]) -> [u32; 96] {
    let mut l_key: [u32; 96] = [0; 96];

    let mut lk: [u32; 8] = [0; 8];
    let mut tm: [u32; 8] = [0; 8];
    let mut tr: [u32; 8] = [0; 8];
    ...
    for i in (0..96).step_by(8) {
        ...
        &l_key[i..i+8].copy_from_slice(&lk[..]);
    }

    l_key
}

pub fn encrypt(in_blk: Cast256Block, key_schedule: [u32; 96], out_blk: &mut Cast256Block) {
    let mut blk: Cast256Block = [0; 4];

    &blk.copy_from_slice(&in_blk[..]);

    f_rnd(&mut blk, key_schedule,  0);
    f_rnd(&mut blk, key_schedule,  8);
    f_rnd(&mut blk, key_schedule, 16);
    f_rnd(&mut blk, key_schedule, 24);
    f_rnd(&mut blk, key_schedule, 32);
    f_rnd(&mut blk, key_schedule, 40);
    i_rnd(&mut blk, key_schedule, 48);
    i_rnd(&mut blk, key_schedule, 56);
    i_rnd(&mut blk, key_schedule, 64);
    i_rnd(&mut blk, key_schedule, 72);
    i_rnd(&mut blk, key_schedule, 80);
    i_rnd(&mut blk, key_schedule, 88);

    out_blk.copy_from_slice(&blk[..]);
}

pub fn decrypt(in_blk: Cast256Block, key_schedule: [u32; 96], out_blk: &mut Cast256Block) {
    let mut blk: Cast256Block = [0; 4];

    &blk.copy_from_slice(&in_blk[..]);

    f_rnd(&mut blk, key_schedule, 88);
    f_rnd(&mut blk, key_schedule, 80);
    f_rnd(&mut blk, key_schedule, 72);
    f_rnd(&mut blk, key_schedule, 64);
    f_rnd(&mut blk, key_schedule, 56);
    f_rnd(&mut blk, key_schedule, 48);
    i_rnd(&mut blk, key_schedule, 40);
    i_rnd(&mut blk, key_schedule, 32);
    i_rnd(&mut blk, key_schedule, 24);
    i_rnd(&mut blk, key_schedule, 16);
    i_rnd(&mut blk, key_schedule,  8);
    i_rnd(&mut blk, key_schedule,  0);
