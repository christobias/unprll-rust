...
pub fn sign(message_hash: Hash256, inputs: &[((SecretKey, SecretKey), u64, usize)], outputs: &[(PublicKey, u64, SecretKey)], tx_fee: u64, mix_ring: Matrix<(PublicKey, PublicKey)>) -> Result<RingCTSignature, failure::Error> {
    ...
    if inputs.len() != mix_ring.rows() {
        return Err(format_err!("Number of secret keys does not match mixin ring"));
    }
    ...
    for (((destination, amount, amount_key), mask), commitment) in outputs.iter().zip(output_masks).zip(signature.bulletproofs[0].V.iter()) {
        sum_out_masks += mask;
        signature.base.output_commitments.push((*destination, commitment.mul_by_cofactor().compress()));
        ...
        match signature.base.signature_type {
            RingCTType::Bulletproof2 => {
                ...
                signature.base.ecdh_exchange.push(ECDHTuple {
                    amount: (SecretKey::from(*amount)).as_bytes()
                        .iter()
                        .zip(amount_mask.as_bytes()).map(|(a, m)| a ^ m)
                        .collect(),
    ...
    for (_, amount, _) in inputs.iter().take(inputs.len() - 1) {
        ...
        signature.input_commitments.push(commitment.into_public().compress());
    }
    ...
    signature.input_commitments.push(in_commit.into_public().compress());
    ...
    ).map(|(mix_row, ((destination_secret_key, commitment_secret_key), _, index), input_mask, input_commitment)| {
        let mlsag_matrix = Matrix::from_iter(
            mix_row.len(),
            2,
            mix_row.iter()
            .flat_map(move |(destination, commitment)| {
                vec![*destination, (commitment.decompress().unwrap() - input_commitment.decompress().unwrap()).compress()]
            })
        );
        ...
        signature.mlsag.iter()
            ...
            .filter_map(|((mlsag, mix_row), input_commitment)| {                
                ...
                let mlsag_matrix = Matrix::from_iter(
                    ...
                    mix_row.iter()
                        .flat_map(move |(destination, commitment)| {
                            vec![*destination, (commitment.decompress().unwrap() - input_commitment).compress()]
                        })
                );
...
mod test {
    ...
    fn it_creates_ringct_signatures_correctly() {
        ...
        for i in 1..=3 {
            ...
            // a
            inputs.push(((destination_keypair.secret_key, commitment.mask), amount, index));
        ...
        let mix_ring = Matrix::from_iter(
            ...
            10,
            keys_commitments.iter().zip(inputs.iter()).flat_map(|((keypair, commitment), (_, _, index))| {
                // 10 Member Ring
                (0..10).clone().map(move |i| {
                    if i == *index {
                        (keypair.public_key, *commitment)
                    } else {
                        (crypto::KeyPair::generate().public_key, crypto::KeyPair::generate().public_key)
                    }
                })
            })
        );

        let signature = ringct::sign(
            message_hash,
            &inputs,
            &destinations,
            3,
